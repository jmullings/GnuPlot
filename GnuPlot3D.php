...
class GnuPlot3D {
    ...
    public function setArrowFrom()
    {
        $base =array(0,0,0);
        $args = func_get_args();
        if (count($args)==1) {

            $this->arrow[] = array_merge($base,$args[0]);
        }else
            $this->arrow[] = array_merge($args[0],$args[1]);
    ...
    public function setZRange($min, $max)
    {
        $this->zrange = array($min, $max);
    ...
    private function maxData($arrow)
    {
    ...
    protected function sendInit()
    {
        ...
        if (is_array($this->arrow)) {
            ...
            $this->sendCommand('set xrange ['.~$maxData.':'.$maxData.']');

            $this->sendCommand('set yrange ['.~$maxData.':'.$maxData.']');

            $this->sendCommand('set zrange ['.~$maxData.':'.$maxData.']');
            $this->sendCommand('set key off');
            $this->sendCommand('splot \'-\' w p ls 1');
            $this->sendCommand('2 2 2');
            $this->sendCommand('-2 -2 2 ');
            $this->sendCommand('e pause -1 ');
        }
        ...
        if ($this->zlabel) {
            $this->sendCommand('set zlabel "'.$this->zlabel.'"');
        }
        ...
        if ($this->zrange) {
            $this->sendCommand('set zrange ['.$this->zrange[0].':'.$this->zrange[1].']');
